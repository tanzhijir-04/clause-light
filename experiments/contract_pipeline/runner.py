...
def load_manifest(path: str | Path, *, require_comparison_set: bool = True) -> list[dict]:
    ...
    with path.open(encoding="utf-8") as handle:
        for line_number, raw in enumerate(handle, 1):
            ...
            if input_path == EXAMPLE_SENTINEL and example_mode:
                pass
            else:
        ...
        for record in records:
            for mode in config["modes"]:
                for repeat_index in range(1, config["runs_per_sample"] + 1):
                    ...
                    stable_key = (record["sample_id"], mode, repeat_index)
                    if any(key[1:] == stable_key for key in existing):
                        continue
                    trace_records = []
                    gateway = LLMGateway(
                    ...
                    ) if mode != "single_pass" else await run_mode(
                        mode,
                        Path(record["input_path"]).read_text(encoding="utf-8"),
                        gateway,
                        sample_id=record["sample_id"],
                        independent_group=record["independent_group"],
                        repeat_index=repeat_index,
